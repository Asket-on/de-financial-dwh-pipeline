...
`docker-compose.postgres.yml` verifies the publication-oriented warehouse SQL against a PostgreSQL target with real `staging` and `dwh` schemas. This covers the schema-capable SQL surface separately from the zero-dependency SQLite adapter.
